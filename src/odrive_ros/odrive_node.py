...
class ODriveNode(object):
    ...
    def __init__(self):
        ...
        self.publish_joint_angles = get_param('publish_joint_angles', False) # if self.sim_mode else False
        ...
        self.publish_raw_odom =get_param('~publish_raw_odom', False)
        ...
        self.serial_number  = get_param('~serial_number', "0")
    ...
    def fast_timer(self, timer_event):
        ...
        if self.fast_timer_comms_active:
            try:
                ...
                if error_string:
                    ...
                    self.m_s_to_value = 1/self.tyre_circumference # calculated
                    #rospy.loginfo("m_s_to_value % 2.2f " % (self.m_s_to_value))
        ...
        try:
            ...
            #if self.fast_timer_comms_active:
            if self.driver:
                if (time_now - self.last_cmd_vel_time).to_sec() > 5.0 and self.last_speed > 0:
        ...
        if self.fast_timer_comms_active and not self.command_queue.empty():
            ...
            if motor_command[0] == 'drive':
                try:
                    ...
                    #rospy.loginfo("Drive Command left % 2.2f , right % 2.2f" % (left_linear_val, right_linear_val))                    
    ...
    def connect_driver(self, request):
        ...
        self.driver = ODriveInterfaceAPI(logger=ROSLogger(), serial_number=self.serial_number)
        ...
        # okay, connected, 
        self.m_s_to_value = 1/self.tyre_circumference
    ...
    def convert(self, forward, ccw):
        ...
        left_linear_val  = (forward - angular_to_linear) * self.m_s_to_value
        right_linear_val = (forward + angular_to_linear) * self.m_s_to_value
    ...
    def pub_odometry(self, time_now):
        ...
        # self.m_s_to_value = 1/tyre_circumference set earlier
